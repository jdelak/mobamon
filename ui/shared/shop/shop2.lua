...
local function gameShopRegisterCategoryTab(object, index, category, useGlow)
	...
	local glowFunc = function()
		while (true) do
			if (not (selectGlow and selectGlow:IsValid())) then break end
			selectGlow:FadeIn(pulseDuration)
			wait(pulseDuration)
			if (not (selectGlow and selectGlow:IsValid())) then break end
			selectGlow:FadeOut(pulseDuration)
			wait(pulseDuration)
...
local function gameShopRegisterItemList(object)
	...
	listContainer:RegisterWatchLua('shopVisControls', function(widget, groupTrigger)
		...
		if (mainShop.title ~= "") then
			object:GetWidget('gameShopSelectRecipeLabel'):SetText(Translate(mainShop.title))
			object:GetWidget('gameShopSelectRecipeLabel'):SetVisible(true)
		elseif trigger_gamePhase.gamePhase >= 4 then
		...
		if componentID then	-- expanded component
			...
			button:SetCallback('onclick', function(widget)
				...
				if triggerPhase.gamePhase >= 4 then	-- rmm this is controlling whether select recipe via leftclick in crafting
					...
					if LuaTrigger.GetTrigger('mainShop').mode == 1 then --build editing
...
local function gameShopRegister(object)
	...
	container:GetWidget('game_shop_closex'):SetCallback('onclick', function(widget)
		...
		elseif LuaTrigger.GetTrigger('mainShop').mode == 1 then
	...
	buyQueueContainer:RegisterWatchLua('shopVisControls', function(widget, groupTrigger)
		...
		widget:SetVisible((mainShop and mainShop.bookmarksVisible) or ((shopView == 0 or shopView == 1) and trigger_gamePhase.gamePhase >= 4 and not trigger_gamePanelInfo.abilityPanel))
	...
	-- Added here, because mainShop doesn't like being a part of shopVisControls 
	container:RegisterWatchLua('mainShop', function(widget, trigger)
		LuaTrigger.GetTrigger('shopVisControls'):Trigger()
	end)
	...
	container:RegisterWatchLua('shopVisControls', function(widget, groupTrigger)
		...
		elseif gamePhase >= 4 then
			...
			widget:SetVAlign(mainShop.valign)
			widget:SetY(mainShop.y)

			object:GetWidget('shopCategoryButton2'):SetVisible(false)	-- Crafted
			object:GetWidget('shopCategoryButton3'):SetVisible(mainPanelStatus.main == mainUI.MainValues.preGame)	-- Boots
			trigger_gamePanelInfo.mapWidgetVis_shopBootsGlow = false

			if (mainShop.visible) then
				showShop = true
				if mainShop.exclusions ~= "" then
					Shop.SetExclusion(mainShop.exclusions)
				else
					Shop.ClearExclusions()
				end
			end
