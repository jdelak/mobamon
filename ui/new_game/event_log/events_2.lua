...
local function GoldSplashRegister()
	...
	function updateGoldPosition()
		...
		local offset = object:GetWidget('gameEventsGold_offset')
...
function gameEventsRegister(object)
	...
	-- local function checkMinimapSide(force)
		...
		-- if (newSwap ~= oldSwap or (force and newSwap)) then
			-- FlipWidgets(eventsContainer, true, 300, true, true)
		-- end
		-- oldSwap = newSwap
	-- end
	-- checkMinimapSide(true)
	-- eventsContainer:RegisterWatchLua('optionsTrigger',  function(widget, trigger)
		-- checkMinimapSide()
	-- end, false)
	...
	function updateEventsPosition()
		...
		if (swapSides == 1) then
			eventsContainer:SetAlign('right')
			
			for i=1,maxDisplaySlots,1 do
				displaySlots[i].parent:SetAlign('right')
			end
		else
			eventsContainer:SetAlign('left')
			
			for i=1,maxDisplaySlots,1 do
				displaySlots[i].parent:SetAlign('left')
			end
		end
	end
...
local function TipEventsRegister(object)
	...
	local self_top_announcements_0 				= 	object:GetWidget('self_top_announcements_0')
	local self_top_announcements_label_0 		= 	object:GetWidget('self_top_announcements_label_0')
	local self_top_announcements_0_wrapper 		= 	object:GetWidget('self_top_announcements_0_wrapper')	
	...
	local function ShowTopTip(stringKey, duration, extraText, extraTextValue)
		
		if (not self_top_announcements_0:IsVisible()) then

			self_top_announcements_0:SetVisible(0)
			
			if extraText then
				self_top_announcements_label_0:SetText(Translate(stringKey, extraText, extraTextValue))
			else
				self_top_announcements_label_0:SetText(Translate(stringKey))
			end
			
			self_top_announcements_0_wrapper:SetHeight(0)
			self_top_announcements_0_wrapper:SetWidth(0)
			self_top_announcements_0_wrapper:SetVisible(1)
			
			self_top_announcements_0_wrapper:SetY('8.4h')	
			
			self_top_announcements_0_wrapper:Scale('35h', '3.5h', 125)		
			
			self_top_announcements_0:FadeIn(250)
			
			self_top_announcements_0:Sleep(duration, function(widget)
				widget:FadeOut(250)
			end)
			
			self_top_announcements_0_wrapper:Sleep(duration, function(widget)
				widget:FadeOut(250)
			end)

			self_top_announcements_0:SetCallback('onclick', function()
				self_top_announcements_0:FadeOut(250)
				self_top_announcements_0_wrapper:FadeOut(250)
			end)
			self_top_announcements_0:SetCallback('onrightclick', function()
				self_top_announcements_0:FadeOut(250)
				self_top_announcements_0_wrapper:FadeOut(250)
			end)			
			self_top_announcements_0:RefreshCallbacks()
			self_top_announcements_0:SetNoClick(0)
			
		end
	end	
	
	self_top_announcements_0:RegisterWatchLua('Team', function(widget, trigger)
		local shouldShow = not ((heroEntity == 'Hero_CapriceTutorial') or (heroEntity == 'Hero_CapriceTutorial2') or (GetCvarString('_gameMapName') == "counter-strife"))
		self_top_announcements_0:UnregisterWatchLua('ScoreboardTeam1')
		self_top_announcements_0:UnregisterWatchLua('ScoreboardTeam2')
		local ScoreTrigger = LuaTrigger.GetTrigger('ScoreboardTeam' .. trigger.team)
		if (ScoreTrigger) then
			local lastScore = ScoreTrigger.roundScore
			self_top_announcements_0:RegisterWatchLua('ScoreboardTeam' .. trigger.team, function(widget, trigger)
				if (trigger.roundScore % 3 == 1) and (lastScore ~= trigger.roundScore) then
					ShowTopTip('game_context_toptip_1', 4500)
				elseif (trigger.roundScore % 3 == 2) and (lastScore ~= trigger.roundScore) then
					ShowTopTip('game_context_toptip_2', 4500)
				elseif ((trigger.roundScore >= 3) and (trigger.roundScore % 3 == 0)) and (lastScore ~= trigger.roundScore) then
					ShowTopTip('game_context_toptip_3', 4500)					
				end
				lastScore = trigger.roundScore
			end, false, 'roundScore')
		end
	end)
