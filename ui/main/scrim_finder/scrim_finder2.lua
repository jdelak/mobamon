...
local function ScrimFinderInit(object)
	...
	local FriendStatusTriggerUI = LuaTrigger.GetTrigger('FriendStatusTriggerUI')
	if (FriendStatusTriggerUI) then
		GetWidget('scrim_finder_chat'):UnregisterWatchLua('FriendStatusTriggerUI')
		GetWidget('scrim_finder_chat'):RegisterWatchLua('FriendStatusTriggerUI', function(widget, trigger)
			if (trigger.friendMultiWindowOpen) then
				widget:SetWidth('812s')
				GetWidget('scrim_finder_no_connection_overlay_blackout'):SetWidth('812s')
			else
				widget:SetWidth('512s')
				GetWidget('scrim_finder_no_connection_overlay_blackout'):SetWidth('512s')
			end
		end, false, nil, 'friendMultiWindowOpen', 'friendLauncherWindowOpen')
		if (FriendStatusTriggerUI.friendMultiWindowOpen) then
			GetWidget('scrim_finder_chat'):SetWidth('812s')
			GetWidget('scrim_finder_no_connection_overlay_blackout'):SetWidth('812s')
		else
			GetWidget('scrim_finder_chat'):SetWidth('512s')
			GetWidget('scrim_finder_no_connection_overlay_blackout'):SetWidth('512s')
		end		
	end		
	...
	scrim_finder:RegisterWatchLua('mainPanelAnimationStatus', function(widget, trigger)
		...
		if animState == 1 then
			if (widget:IsVisible()) then
				...
				elseif (Friends) and (Friends.ToggleFriendsList) then
					Friends.ToggleFriendsList(false, nil, true)
				end				
			end			
			libThread.threadFunc(function()	
		...
		elseif animState == 3 then	
			local function goBackToModeSelect()
				libThread.threadFunc(function()
					wait(1)				
					local triggerPanelStatus = LuaTrigger.GetTrigger('mainPanelStatus')
					triggerPanelStatus.main = mainUI.MainValues.selectMode
					triggerPanelStatus:Trigger(false)
				end)
			end
			local breadCrumbsTable = {
				{text='partystatus_queue_change_mode', onclick=goBackToModeSelect},	
				{text='partystatus_queue_name_scrim', onclick=function() end},	
			}
			mainUI.initBreadcrumbs(breadCrumbsTable, nil)
