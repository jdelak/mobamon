...
GameUI.RadialSelection.RadialSelectionOpen 		= {['command'] = false, ['chat'] = false}
...
function GameUI.RadialSelection:hide(radialType)
	...
	GameUI.RadialSelection.RadialSelectionOpen[radialType] = false
	oldY[radialType] = nil
	oldX[radialType] = nil
	GameUI.RadialSelection.visible = false
end
...
function GameUI.RadialSelection:create(radialType, selectionList, mouseXOffset, mouseYOffset, pingCoords, scale, displayLabels, displayTitle)
	local scale = scale or 1
	-- false or true equates to true, so we have to explicitly check for nil here
	if displayLabels == nil then displayLabels = true end
	if displayTitle  == nil then displayTitle  = true end
	...
	GameUI.RadialSelection.visible = true
	...
	local divider = interface:GetWidget('radial_selection_'..radialType..'_divider')
	local titleUnderline = interface:GetWidget('radial_'..radialType..'_description_underline')
	local numItems = #selectionList -1 -- works because elements should be numerical
	local buttons = {}
	local subButtons = {}
	local radius = 26*scale -- in h
	local iconRadius = 22*scale -- in h
	local centerRadius = 75*scale
	local sectionRadius = libGeneral.HtoP(15*scale)
	local subSectionIconRadius = 40*scale -- in h
	local subSectionIconsize = 8*scale -- in h
	...
	local bounds = {libGeneral.HtoP(21*scale), libGeneral.HtoP(21*scale), GetScreenWidth()-libGeneral.HtoP(21*scale), GetScreenHeight()-libGeneral.HtoP(21*scale)}
	desiredX = clamp(desiredX, bounds[1], bounds[3])
	desiredY = clamp(desiredY, bounds[2], bounds[4])
	
	-- Make the drop-shadow work
	local size = libGeneral.HtoP(radius*scale*(1.4/scale))
	for n = 1, 100 do
		local parentWidget = interface:GetWidget('radial_selection_'..radialType..'_shadow_'..n..'_parent')
		if (not (parentWidget and parentWidget:IsValid())) then break end
		parentWidget:DoEvent() -- Move clipping parent to where it needs to be

		local widget = interface:GetWidget('radial_selection_'..radialType..'_shadow_'..n) -- Move circular background child to the mouse, to be clipped
		widget:SetWidth(size)
		widget:SetHeight(size)
		widget:SetAbsoluteX(desiredX-size/2)
		widget:SetAbsoluteY(desiredY-size/2)
	end
	
	selection_mover:SetX(-centerPos[1] + desiredX)
	selection_mover:SetY(-centerPos[2] + desiredY)
	
	divider:SetWidth(1.1*radius..'h')
	divider:SetHeight(1.1*radius..'h')
	centerCircle:SetWidth(11.8*scale..'h')
	centerCircle:SetHeight(11.8*scale..'h')
	centerCircleIcon:SetWidth(4.5*scale..'h')
	centerCircleIcon:SetHeight(4.5*scale..'h')
	...
	centerCircle:SetCallback('onclick', function(widget)
		...
		if (selectionList[numItems+1].ping) then
	...
	centerCircle:SetCallback('onmouseover', function(widget)
		...
		centerCircleIcon:Scale(6*scale..'h', 6*scale..'h', 50)
	...
	centerCircle:SetCallback('onmouseout', function(widget)
		centerCircleIcon:Scale(4.5*scale..'h', 4.5*scale..'h', 50)
	end)
	...
	titleUnderline:SetVisible(displayTitle)
	...
	for n=1, numItems do
		...
		local icony = -(iconRadius)/2*sin(rotation) - 1
		local descy = icony + 5
		local glowRadius = radius/2.3 + 14
		local nameWidth = radius/2.3 + 5
		local nameHeight = radius/2.3 + 2
		local size = 'maindyn_12'
		...
		local button = selection_container:InstantiateAndReturn('radial_selection_item', 
			...
			'nameWidth', nameWidth..'h', 
			'nameHeight', nameHeight..'h',
			'texture', selectionList[n].texture, 
			'visible', selectionList[n].visible, 
			'description', displayLabels and selectionList[n].desc or '',
			'size', size,
			'max', numItems, 
	...
	GameUI.RadialSelection.RadialSelectionOpen[radialType] = true
	...
	radial_selection:RegisterWatchLua('System', function(widget, trigger)
		...
		if not GameUI.RadialSelection.RadialSelectionOpen[radialType] then -- menu is closed, time to stop
			...
			movingCenterThread = libThread.threadFunc(function()				
				...
				if (centerCircle) and (centerCircle:IsValid()) then
					...
					local pixelRadius = libGeneral.HtoP(20*scale)
					local selectedPixelRadius = libGeneral.HtoP(25*scale)
					local innerRadius = libGeneral.HtoP(7*scale)
	...
	function GameUI.RadialSelection.confirmSelection()
...
function GameUI.RadialSelection.setAreaToWidget(widget, otherWidgetName)
	local otherWidget = interface:GetWidget(otherWidgetName)
	if not otherWidget then
		-- println("warning, no widget: " .. tostring(otherWidgetName))
		return
	end
	-- println("Setting " .. widget:GetName() ..  " to " .. otherWidget:GetAbsoluteX() .. " " .. otherWidget:GetAbsoluteY() .. " " .. otherWidget:GetWidth() .. " " .. otherWidget:GetHeight())
	widget:SetX(otherWidget:GetAbsoluteX())
	widget:SetY(otherWidget:GetAbsoluteY())
	widget:SetWidth(otherWidget:GetWidth())
	widget:SetHeight(otherWidget:GetHeight())
end