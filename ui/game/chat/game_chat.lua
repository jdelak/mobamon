	...
	local function addEntry(message, messageColorR, messageColorG, messageColorB, senderName, playerNameColorR, playerNameColorG, playerNameColorB, entityIcon, chatType, identID, playerIndex)
		...
		if displaySlot then
			...
			if (identID) and identID ~= myIdentId and (playerIndex) then
				...
				for i = 0, 4 do
					if identID and LuaTrigger.GetTrigger(triggerPrefix..i) and LuaTrigger.GetTrigger(triggerPrefix..i).identID then
						if string.gsub(tostring(LuaTrigger.GetTrigger(triggerPrefix..i).identID), "%.", "") and tonumber((string.gsub(tostring(LuaTrigger.GetTrigger(triggerPrefix..i).identID), "%.", ""))) and tonumber((string.gsub(tostring(LuaTrigger.GetTrigger(triggerPrefix..i).identID), "%.", ""))) == identID then
							index = i
							break
						end
					end
				end
				local clientNumber = playerIndex
				if (clientNumber > 4) then
					clientNumber = clientNumber - 5
				end
				local identTrigger = LuaTrigger.GetTrigger(triggerPrefix..index)
				local ident
				if (identTrigger) then
					ident = identTrigger.identID
				end
				
				displaySlot.Mute:SetCallback('onclick', function(widget, showChatHistory)
					if (identTrigger) and (ident) then
						UIMute(index, ident, clientNumber, LuaTrigger.GetTrigger(triggerPrefix..index).name, '0') -- RMM I think this index is the wrong one for mute
						
						local muteTrigger = LuaTrigger.GetTrigger('mutePlayerInfo')
						muteTrigger.IdentID = ident
						muteTrigger.muted = ChatClient.IsIgnored(tostring(ident))
						muteTrigger:Trigger()
					end
				end)
				if (identTrigger) and (ident) then
	...
	function AllStrifeChatMessages(widget, trigger)
		...
		if senderRelation == 1 or senderRelation == 4 or messageType == 2 or senderRelation == 5 then
			...
			if (trigger) and (trigger.clanTag) and (not Empty(trigger.clanTag)) then
				senderName = (('[' .. (trigger.clanTag or '') ..']') .. (senderName or ''))
			end				
		...
		if (gamePhaseTrigger.gamePhase == 7) then
			...
			for k,v in pairs(containers) do
				...
				v:RegisterWatchLua('gamePanelInfo', function(widget, trigger)
					...
					if (trigger.moreInfoKey or trigger_gamePanelInfo.heroVitalsVis) and (not GetCvarBool('cg_newHotnessUI')) then
						targY = targY - 3.8
					end

					if (trigger.lanePusherVis) and (not GetCvarBool('cg_newHotnessUI')) then
						targY = targY - 3.8
					end			

					if (trigger.channelBarVis) or (trigger.respawnBarVis) then
						if GetCvarBool('cg_newHotnessUI') then
							targY = targY - 6.8
						else
							targY = targY - 3.8
						end
