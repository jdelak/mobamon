...
function registerLevelUpEntry(object, index)
	...
	local frame_3				= object:GetWidget('abilityLevelUpEntry'..index..'_frame_1')
	...
	local simple_frame_3		= object:GetWidget('abilityLevelUpEntry'..index..'_simple_frame_3')
	...
	local function levelUpClick(widget)
		...
		--[[
		if triggerUnit.availablePoints == 1 then
	...
	button:SetCallback('onmouseover', function(widget)
		frame_1:SetColor('#6d5d4a')
		frame_1:SetBorderColor('#6d5d4a')
		-- frame_2:SetColor('#121111')
		-- frame_2:SetBorderColor('#121111')
		-- frame_3:SetColor('#555555')
		-- frame_3:SetBorderColor('#555555')	
	end)
	
	button:SetCallback('onmouseout',  function(widget)
		frame_1:SetColor('#4e453b')
		frame_1:SetBorderColor('#4e453b')
		-- frame_2:SetColor('black')
		-- frame_2:SetBorderColor('black')
		-- frame_3:SetColor('#333333')
		-- frame_3:SetBorderColor('#333333')		
	end)	
	
	buttonSimple:SetCallback('onmouseover', function(widget)
		simple_frame_1:SetColor('#6d5d4a')
		simple_frame_1:SetBorderColor('#6d5d4a')
		simple_frame_2:SetColor('#121111')
		simple_frame_2:SetBorderColor('#121111')
		simple_frame_3:SetColor('#555555')
		simple_frame_3:SetBorderColor('#555555')	
	end)
	
	buttonSimple:SetCallback('onmouseout',  function(widget)
		simple_frame_1:SetColor('#4e453b')
		simple_frame_1:SetBorderColor('#4e453b')
		simple_frame_2:SetColor('0 0 0 1')
		simple_frame_2:SetBorderColor('0 0 0 1')
		simple_frame_3:SetColor('#333333')
		simple_frame_3:SetBorderColor('#333333')		
	end)
	...
	frame_1:RegisterWatchLua('HeroInventory'..index, function(widget, trigger)
		if (trigger.canLevelUp) then
			widget:SetColor('#4e453b')
			widget:SetBorderColor('#4e453b')
		else
			widget:SetColor('#121111')
			widget:SetBorderColor('#121111')
		end
	end, true, nil, 'canLevelUp', 'level')		
	
	simple_frame_1:RegisterWatchLua('HeroInventory'..index, function(widget, trigger)
		if (trigger.canLevelUp or trigger.level > 0) then
			widget:SetColor('#4e453b')
			widget:SetBorderColor('#4e453b')
		else
			widget:SetColor('#121111')
	...
	--[[
	panelButton:RegisterWatchLua('HeroInventory'..index, function(widget, trigger) widget:SetEnabled(trigger.canLevelUp) end, false, nil, 'canLevelUp')
	--]]
	
	-- levelUpButton:RegisterWatchLua('HeroInventoryStatus'..index, function(widget, trigger) widget:SetEnabled(trigger.canLevelUp) end)
	...
	manaCost:RegisterWatchLua('HeroInventory'..index, function(widget, trigger)
		if trigger.canLevelUp then
			...
			manaCostSimple:SetColor('#00CCFF')
		else
			widget:SetColor(0.5, 0.5, 0.5)
			manaCostSimple:SetColor(0.5, 0.5, 0.5)
		end
	end, true, nil, 'canLevelUp')
	...
	manaIcon:RegisterWatchLua('HeroInventory'..index, function(widget, trigger)
		if trigger.canLevelUp then
			widget:SetColor('#00CCFF')
			manaIconSimple:SetColor('#00CCFF')
		else
			widget:SetColor(0.5, 0.5, 0.5)
			manaIconSimple:SetColor(0.5, 0.5, 0.5)
		end
	end, true, nil, 'canLevelUp')
	...
	container:RegisterWatchLua('HeroInventory'..index, function(widget, trigger)
		...
		if trigger.canLevelUp then
			...
			description:SetColor(colorGray)
			descriptionSimple:SetColor(colorGray)
			cooldownLabel:SetColor(colorGray)
			name:SetColor(colorGray)
			nameSimple:SetColor(colorGray)
			levelHeader:SetColor(colorGray)

			for i=1,4,1 do
				levelHeadings[i]:SetColor(colorGray)
			end
		end
	end, false, nil, 'canLevelUp')
...
function registerLevelUpPanel(object)
	...
	listSimple:RegisterWatchLua('gamePanelInfo', function(widget, trigger) widget:SetVisible(trigger.abilityPanelView == 0) end, false, nil, 'abilityPanelView')
	...
	local function abilityKeySwap()
		--[[
		for i=0,3,1 do
			if keysActivate[i] and string.len(keysActivate[i]) > 0 and keysLevel[i] and string.len(keysLevel[i]) > 0 then
				-- print('BindImpulse game '..keysLevel[i]..' ActivateTool '..i..'\n')
				-- print('BindImpulse game '..keysActivate[i]..' LevelupAbility '..i..'\n')
				Cmd('Unbind game '..keysActivate[i])
				Cmd('Unbind game '..keysLevel[i])
				Cmd('BindImpulse game INVALID ActivateTool '..i)
				Cmd('BindImpulse game INVALID LevelupAbility '..i)
				Cmd('BindImpulse game '..keysLevel[i]..' ActivateTool '..i)
				Cmd('BindImpulse game '..keysActivate[i]..' LevelupAbility '..i)
			end
		end
	...
	local function abilityKeyUnswap()
		...
		for i=0,3,1 do
			if keysActivate[i] and string.len(keysActivate[i]) > 0 and keysLevel[i] and string.len(keysLevel[i]) > 0 then
				-- print('BindImpulse game '..keysActivate[i]..' ActivateTool '..i..'\n')
				-- print('BindImpulse game '..keysLevel[i]..' LevelupAbility '..i..'\n')
				Cmd('Unbind game '..keysActivate[i])
				Cmd('Unbind game '..keysLevel[i])
				Cmd('BindImpulse game INVALID ActivateTool '..i)
				Cmd('BindImpulse game INVALID LevelupAbility '..i)
				Cmd('BindImpulse game '..keysActivate[i]..' ActivateTool '..i)
				Cmd('BindImpulse game '..keysLevel[i]..' LevelupAbility '..i)
			end
		end
		object:UICmd("Refresh()")
		--]]
	...
	-- libGeneral.createGroupTrigger('levelUpButtonPos', { 'heroHealthVis', 'heroManaVis', 'ModifierKeyStatus', 'channelBarVis' })
	
	--[[
	buttonContainer:RegisterWatchLua('levelUpButtonPos', function(widget, groupTrigger)
		...
		local targY				= styles_heroLevelUpButtonBaseY
	...
	buttonContainer:RegisterWatchLua('HeroUnit', function(widget, trigger) widget:SetVisible(trigger.availablePoints > 0) end, false, nil, 'availablePoints')
	...
	-- Moved to the game2.lua file to use the generic "gameRegisterBindableHotkey" functionality
	-- openButtonKey:RegisterWatchLua('ModifierKeyStatus', function(widget, trigger) widget:SetEnabled(trigger.moreInfoKey) end)

	-- openButtonKeyBacker:RegisterWatchLua('ModifierKeyStatus', function(widget, trigger)
	...
	-- object:GetWidget('abilitiesLevelUpButtonKey'):SetCallback('onclick', function(widget)
