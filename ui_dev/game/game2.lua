...
local PRIMARY_ABILITIES_START 	= 0
local PRIMARY_ABILITIES_END 	= 6
local SHARED_ABILITIES_START 	= 64
local SHARED_ABILITIES_END 		= 66
...
function registerRadialCommand(object)
	...
	masterWidget:RegisterWatchLua('PingWheelInfo', function(widget, trigger)
		...
		local worldPos = trigger.worldPos
		openRadialCommand(1, Input.GetCursorPosX()-startX, Input.GetCursorPosY()-startY, worldPos)
	end)
end
...
function registerRadialChat(object)
	...
	masterWidget:RegisterWatchLua('ModifierKeyStatus', function(widget, trigger)
		...
		if (trigger.alt) then
			...
			GameUI.RadialSelection:hide('command')
		end
	end)
end
...
local function inventoryRegisterPrimary(object, index) -- hero primary abilities
	...
	for k,v in pairs(object:GetGroup('gameInventory'..index..'Containers')) do
		v:RegisterWatchLua('SPEAbilityUpdate', function(widget, trigger)
			if trigger['ShowActiveAbility'..index] then
...
for i=PRIMARY_ABILITIES_START,PRIMARY_ABILITIES_END,1 do
	inventoryRegisterPrimary(object, i)
end

for i=SHARED_ABILITIES_START,SHARED_ABILITIES_END,1 do
	inventoryRegisterPrimary(object, i)
end
...
local function gameRegister(object)
	...
	object:GetWidget('gameMenuPing'):SetCallback('onclick', function()
		if GetLocalHeroLocation then
			...
			local startY = tonumber(string.sub(trigger.mousePos,    string.find(trigger.mousePos, " ")+1))
			if (worldPos) then
				openRadialCommand(1, 0, 0, worldPos)
			end
		end
	end)
...
function UIMute(slotIndex, identID, clientNumber, name, uniqueID)
	...
	local heroIndex = GetTrigger('AllyUnit'..slotIndex)
	if heroIndex then 
	...
	if (thisGuyWasMuted) then
		...
		UnMutePlayerPings(clientNumber)
