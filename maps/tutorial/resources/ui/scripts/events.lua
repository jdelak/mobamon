...
function gameEventsRegister(object)
	...
	local function populateDisplaySlotKill(displaySlot, eventData)
		...
		displaySlot.icon1:SetTexture('/ui/dev_elements/game_temp/death.tga')
		...
		displaySlot.icon1:SetWidth('46@')
		displaySlot.icon1:SetHeight('46%')
	...
	eventsContainer:RegisterWatchLua('EventKill', function(widget, trigger)
		...
		if killType ~= 1 and killType ~= 2 then
			return
		end
		...
		if displaySlot then
			populateDisplaySlotLaneChoice(
				displaySlot,
				{
					...
					pushMessage		= GetEntityDisplayName(entityName)..'^w will spawn ^*'..fullLaneNames[laneName],
					notifyColorR		= notifyColorR,
					notifyColorG		= notifyColorG,
					notifyColorB		= notifyColorB,
					backerColor			= backerColor
...
function ArcadeEventsRegister(object)
	...
	function Side1ArcadeEventKill(trigger, isDelayed)
		...
		if (trigger.killerTeam == 1) and (not trigger.killerIsSelf) then
			...
			interface:GetWidget('game_team_announcements_1_sleeper'):Sleep(announcerFadeOutDelay * 0.30, function()
				...
				if (team == 1) and (trigger.playerGoldIncome > 0) then
					interface:GetWidget('game_team_announcements_1_line_3'):FadeIn(gameEvents.longFade)
					-- gold
	...
	function Side1ArcadeEventBuildingKill(trigger, isDelayed, isBoss)
		...
		if (not trigger_gamePanelInfo.mapWidgetVis_arcadeText) or ((trigger.attackerTeam ~= 1) or ((trigger.ownerTeam ~= 2) and (not isBoss))) or ((not isBoss) and (not isTowerEntity(trigger.entityName)) and (not isBarracksEntity(trigger.entityName))) then
			return
		end
	...
	function Side2ArcadeEventKill(trigger, isDelayed)
		...
		if (trigger.killerTeam == 2) and (not trigger.killerIsSelf) then
			...
			interface:GetWidget('game_team_announcements_2_sleeper'):Sleep(announcerFadeOutDelay * 0.30, function()
				...
				if (team == 2) and (trigger.playerGoldIncome > 0) then
					interface:GetWidget('game_team_announcements_2_line_3'):FadeIn(gameEvents.longFade)
					-- gold
					interface:GetWidget('game_team_announcements_2_line_3_b'):SetVisible(1)
				
					-- interface:GetWidget('game_team_announcements_2_line_3_label_2'):RegisterWatchLua('EventHeroGold', function(widget, trigger2) widget:SetText('+' .. trigger2.gold) end)
					-- interface:GetWidget('game_team_announcements_2_line_3_label_2'):SetText('+' .. LuaTrigger.GetTrigger('EventHeroGold').gold)					
					interface:GetWidget('game_team_announcements_2_line_3_label_2'):SetText('Assist +' .. trigger.playerGoldIncome)					
				else
	...
	function Side2ArcadeEventBuildingKill(trigger, isDelayed, isBoss)
		...
		if (not trigger_gamePanelInfo.mapWidgetVis_arcadeText) or ((trigger.attackerTeam ~= 2) or ((trigger.ownerTeam ~= 1) and (not isBoss))) or ((not isBoss) and (not isTowerEntity(trigger.entityName)) and (not isBarracksEntity(trigger.entityName))) then
			return
		end
		...
		interface:GetWidget('game_team_announcements_2_sleeper'):Sleep(announcerFadeOutDelay * 0.30, function()
			...
			if (LuaTrigger.GetTrigger('EventHeroGold')) then
				...
				if (team == 2) then
					...
					if (isBoss) then
						interface:GetWidget('game_team_announcements_2_line_3_label_2'):RegisterWatchLua('EventBossGold', function(widget, trigger2) widget:SetText('+' .. trigger2.gold) end)
						interface:GetWidget('game_team_announcements_2_line_3_label_2'):SetText('+' .. LuaTrigger.GetTrigger('EventBossGold').gold)					
					elseif isTowerEntity(trigger.entityName) then
						interface:GetWidget('game_team_announcements_2_line_3_label_2'):RegisterWatchLua('EventTowerGold', function(widget, trigger2) widget:SetText('+' .. trigger2.gold) end)
						interface:GetWidget('game_team_announcements_2_line_3_label_2'):SetText('+' .. LuaTrigger.GetTrigger('EventTowerGold').gold)
					else
						interface:GetWidget('game_team_announcements_2_line_3_label_2'):RegisterWatchLua('EventGeneratorGold', function(widget, trigger2) widget:SetText('+' .. trigger2.gold) end)
						interface:GetWidget('game_team_announcements_2_line_3_label_2'):SetText('+' .. LuaTrigger.GetTrigger('EventGeneratorGold').gold)
					end
				else
...
local function TipEventsRegister(object)
	...
	local function ShowTip(stringKey, duration)
