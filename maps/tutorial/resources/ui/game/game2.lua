...
function registerRadialCommand(object)
	...
	local masterWidget = object:GetWidget("radial_selection")
	
	masterWidget:RegisterWatchLua('ModifierKeyStatus', function(widget, trigger)
		...
		if (trigger.alt) then
			...
			waitingThread = libThread.threadFunc(function()
				for i = 1, floor(radialWaitTime/20) do
					wait(20)
					local mouseDistX = Input.GetCursorPosX()-mouseX
					local mouseDistY = Input.GetCursorPosY()-mouseY
					-- Wait, or, if we drag, end early
					if mouseDistX*mouseDistX + mouseDistY*mouseDistY > cursorDragDetectAmount*cursorDragDetectAmount then
						break
					end
				end
				openRadialCommand(1, Input.GetCursorPosX()-mouseX, Input.GetCursorPosY()-mouseY)
				waitingThread = nil
			end)
		else
			if (waitingThread) then
...
function registerRadialChat(object)
	local waitingThread = nil
	local rotatingThread = nil
	local masterWidget = object:GetWidget("chat_wheel_menu_master")
	...
	masterWidget:RegisterWatchLua('ModifierKeyStatus', function(widget, trigger)
		...
		if (trigger.ctrl) then
			...
			waitingThread = libThread.threadFunc(function()
				wait(GetCvarNumber('cg_chatWheelOpen'))
				GameUI.ChatWheel:open(0)
				waitingThread = nil
			end)
		else
			if (waitingThread) then
...
for i=0,3,1 do
	...
	object:RegisterWatchLua('gameArrangeInventoryInfo', function(widget, groupTrigger)
		...
		for i=0,3,1 do
			if triggersInventory[i].hasTimer or triggersInventory[i].alwaysShowTimer then
...
local function gamePlayerInfoMuteIconRegister(object, widgetName, unitTrigger, slotIndex)
	...
	local function isMuted(identID)
...
local function heroRegister(object)
	...
	local function ShowHeroTooltip(sourceWidget, relation, index, showInventory)
		...
		if (relation == 'Hero') then
			...
			sourceWidget:GetWidget('game_hero_unitframe_tooltip_player_label'):SetText(unitTrigger.playerName)
		...
		local gameSelfArmor	= object:GetWidget('gameSelfArmor')
		local gameSelfMagicArmor	= object:GetWidget('gameSelfMagicArmor')
		...
		gameSelfPower:RegisterWatchLua(relation..'Unit'..index, function(widget2, trigger2)
			widget2:SetText(trigger2.power)
		...
		gameSelfArmor:SetText(math.ceil(unitTrigger.armor))
		gameSelfArmor:UnregisterWatchLuaByKey('tooltip_a_label')
		gameSelfArmor:RegisterWatchLua(relation..'Unit'..index, function(widget2, trigger2)
			widget2:SetText(math.ceil(trigger2.armor))
		end, false, 'tooltip_a_label', 'armor')	
		
		gameSelfMagicArmor:SetText(math.ceil(unitTrigger.magicArmor))
		gameSelfMagicArmor:UnregisterWatchLuaByKey('tooltip_ma_label')
		gameSelfMagicArmor:RegisterWatchLua(relation..'Unit'..index, function(widget2, trigger2)
			widget2:SetText(math.ceil(trigger2.magicArmor))
		end, false, 'tooltip_ma_label', 'magicArmor')	
